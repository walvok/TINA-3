...
void setup() {
  ...
  pinMode(LOFFPLUS, INPUT); //Setting pins for heart rate senzor
  pinMode(LOFFMINUS, INPUT); //Setting pins for heart rate senzor 
...
void loop(void) {
  ...
  File dataFileGPS = SD.open("datalog.txt", FILE_WRITE);
  if (dataFileGPS) 
    {
      dataFileGPS.println(dataString);
      dataFileGPS.close();
  ...
  //Saving data from hear rate senzor
  String dataStringHR = ""; //Declaration of variable - for saving values on SD card. Saving is by rows  
  //if (period != lastperiod) {
    ...
    //}
  //}   
  dataStringHR += "Frekvence: ";
  dataStringHR += String(frequency);
  dataStringHR += "Udery: ";
  dataStringHR += String(beatspermin);
  File dataFileHR = SD.open("hearrate.txt", FILE_WRITE);
  if (dataFileHR) 
    {
      dataFileHR.println(dataStringHR);
      dataFileHR.close();
      Serial.println(dataStringHR);
    }  
    else 
    {
      Serial.println("error opening datalog.txt");
